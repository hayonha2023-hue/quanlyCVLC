...
def update_kpi_db(shop_id, user_kpi, data):
    return save(f'kpi/{shop_id}/{user_kpi}', data)
...
def render_kpi():
    ...
    scoped = full_db if shop_id == 'Shop Chính (Mặc định)' else full_db.get('shops', {}).get(shop_id, {})
    hunt_kpi({k:v for k,v in scoped.items() if k not in ('shops', 'kpi', 'users', 'settings')})
    ...
    can_edit = current_user.lower() == "admin" or user_role in ("admin", "super_admin") or "SỬA SỐ KPI" in edit_perms
